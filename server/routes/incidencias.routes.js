...
app.get('/traffic', function (req, res) {
   ...
   var file;
   const fs = require('fs');
   file= __dirname + '/trafico/CDMX_2020-01-14-'+times[i]+'.json'
...
app.get('/semaforoizt', function (req, res) {
   ...
   var file;
   const fs = require('fs');
   file= __dirname +'/semaforos_iztapalapa.json'
   let rawdata = fs.readFileSync(file);
...
app.get('/semaforoizc', function (req, res) {
   ...
   var file;
   const fs = require('fs');
   file= __dirname +'/semaforos_iztacalco.json'
   let rawdata = fs.readFileSync(file);
   let semaforo = JSON.parse(rawdata)
   res.json(semaforo);
...
app.get('/semaforomh', function (req, res) {
   ...
   var file;
   const fs = require('fs');
   file= __dirname +'/semaforos_miguelhidalgo.json'
   let rawdata = fs.readFileSync(file);
   let semaforo = JSON.parse(rawdata)
   res.json(semaforo);
...
app.get('/alcaldias', function(req,res){
   var file;
   const fs = require('fs');
   file= __dirname + '/alcaldias.geojson'
   let rawdata = fs.readFileSync(file);
   ...
   app.get('/incidencias', function (req, res) {
      ...
      var file;
      const fs = require('fs');
      file= __dirname + '/incidencias.json'
      let rawdata = fs.readFileSync(file);
